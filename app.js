...
function renderAomoriMap() {
  ...
  map.innerHTML = `
    <div class="map-pan-layer" id="mapPanLayer" style="--pan-x:0px; --pan-y:0px; --map-scale:1">
    ...
    <div class="map-controls" aria-label="地図操作">
      <button type="button" data-map-action="zoom-in" aria-label="地図を拡大">＋</button>
      <button type="button" data-map-action="zoom-out" aria-label="地図を縮小">−</button>
      <button type="button" data-map-action="reset" aria-label="地図位置を戻す">↺</button>
    </div>
    <p class="map-hint">🖐️ ドラッグで動かせるよ・＋−でズーム</p>
    <p class="map-attribution">${TRIP_CONFIG.mapSource.attribution}</p>
  `;

  setupAomoriMapInteraction(map);
}


function setupAomoriMapInteraction(map) {
  const layer = map.querySelector('#mapPanLayer');
  if (!layer) return;
  ...
  const maxPan = () => Math.max(92, Math.min(map.clientWidth, map.clientHeight) * (0.18 + (interaction.scale - 1) * 0.7));

  function applyTransform() {
    const limit = maxPan();
    interaction.x = clamp(interaction.x, -limit, limit);
    interaction.y = clamp(interaction.y, -limit, limit);
    layer.style.setProperty('--pan-x', `${interaction.x}px`);
    layer.style.setProperty('--pan-y', `${interaction.y}px`);
    layer.style.setProperty('--map-scale', interaction.scale.toFixed(2));
  }

  function setScale(nextScale) {
    interaction.scale = clamp(nextScale, 1, 1.85);
    applyTransform();
  }
  ...
  map.addEventListener('pointerdown', (event) => {
    ...
    interaction.originX = interaction.x;
    interaction.originY = interaction.y;
  ...
  map.addEventListener('pointermove', (event) => {
    ...
    if (Math.hypot(dx, dy) > 5) interaction.moved = true;
    interaction.x = interaction.originX + dx;
    interaction.y = interaction.originY + dy;
    applyTransform();
  });
  ...
  map.querySelector('[data-map-action="zoom-in"]')?.addEventListener('click', () => setScale(interaction.scale + 0.18));
  map.querySelector('[data-map-action="zoom-out"]')?.addEventListener('click', () => setScale(interaction.scale - 0.18));
  map.querySelector('[data-map-action="reset"]')?.addEventListener('click', () => {
    interaction.x = 0;
    interaction.y = 0;
    interaction.scale = 1;
    applyTransform();
  ...
  map.addEventListener('wheel', (event) => {
    ...
    event.preventDefault();
    setScale(interaction.scale + (event.deltaY < 0 ? 0.12 : -0.12));
  }, { passive: false });
